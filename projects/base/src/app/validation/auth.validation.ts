...
import { AuthService as AuthServiceAdmin } from "@serviceAdmin/auth.service";
...
export const authValidationNonLoginAdmin = (route: Route, segments: UrlSegment[]) => {
    const auth = inject (AuthServiceAdmin);
    const messageService = inject (MessageService);
    const profile = auth.getProfile();
    const router = inject(Router);
